...
// Note: Assuming these local files exist in your project structure
...
// Mocked constants and utils for standalone functionality
const GAME_CONFIG = {
    MIN_TILE_INDEX: -8,
    MAX_TILE_INDEX: 8,
    TILE_SIZE: 40,
    ...
    INITIAL_ROWS: 30,
    STEP_TIME: 0.2,
    PLAYER_BASE_HEIGHT: 8,
    PLAYER_JUMP_HEIGHT: 20,
};
const TILES_PER_ROW = GAME_CONFIG.MAX_TILE_INDEX - GAME_CONFIG.MIN_TILE_INDEX + 1;
...
function generateRows(count, startingIndex = 0) {
    const rows = [];
    for (let i = 0; i < count; i++) {
        const rowIndex = startingIndex + i;
        const type = Math.random() > 0.4 ? (Math.random() > 0.5 ? 'car' : 'truck') : 'forest';
        if (type === 'forest') {
            rows.push({
                type: 'forest',
                trees: Array.from({ length: Math.floor(Math.random() * 4) + 2 }, () => ({
                    tileIndex: Math.floor(Math.random() * (GAME_CONFIG.MAX_TILE_INDEX - GAME_CONFIG.MIN_TILE_INDEX + 1)) + GAME_CONFIG.MIN_TILE_INDEX,
                    height: Math.random() * 40 + 30
                }))
            });
        } else {
             const speed = (Math.random() * 100 + 80) + getDifficultyLevel(rowIndex) * 20;
             rows.push({
                type: type,
                direction: Math.random() > 0.5,
                speed: speed,
                vehicles: Array.from({ length: Math.floor(Math.random() * 2) + 2 }, () => ({
                    initialTileIndex: Math.floor(Math.random() * (GAME_CONFIG.MAX_TILE_INDEX - GAME_CONFIG.MIN_TILE_INDEX + 1)) + GAME_CONFIG.MIN_TILE_INDEX,
...
function calculateFinalPosition(currentPosition, moves) {
    return moves.reduce((pos, move) => {
        if (move === "forward") pos.rowIndex++;
        if (move === "backward") pos.rowIndex--;
        if (move === "left") pos.tileIndex--;
        if (move === "right") pos.tileIndex++;
        return pos;
    }, { ...currentPosition });
}

function createSwipeDetector(element) {
    ...
    function handleTouchEnd() {
        ...
        if (Math.abs(diffX) > Math.abs(diffY)) { // Horizontal swipe
            if (diffX > 50) swipeCallback('right');
            else if (diffX < -50) swipeCallback('left');
        } else { // Vertical swipe
            if (diffY > 50) swipeCallback('backward');
            else if (diffY < -50) swipeCallback('forward');
        }
    ...
    return {
        onSwipe: (callback) => { swipeCallback = callback; },
        destroy: () => {
...
    // In this environment, we can't redirect, but this is how you would.
...
        ),
        // Belly Patch
        React.createElement("mesh", { position: [0, -7.1, 7] }, // Adjusted Y to be on the front
            React.createElement("boxGeometry", { args: [12, 1, 12] }),
            React.createElement("meshBasicMaterial", { color: 0xD2B48C })
        ),
        // Wings
        React.createElement("mesh", { position: [-8, 0, 6], castShadow: true, receiveShadow: true },
...
        // --- Glasses & Eyes ---
        // Left Eye (white part)
        React.createElement("mesh", { position: [-4, -7.2, 11] },
            React.createElement("boxGeometry", { args: [5, 1, 5] }),
        ...
        // Left Pupil
        React.createElement("mesh", { position: [-4, -7.3, 11] },
            React.createElement("boxGeometry", { args: [2, 1, 2] }),
            React.createElement("meshBasicMaterial", { color: 0x000000 })
        ),
        // Right Pupil
        React.createElement("mesh", { position: [4, -7.3, 11] },
            React.createElement("boxGeometry", { args: [2, 1, 2] }),
...
        // Beak
        React.createElement("mesh", { position: [0, -7.5, 5], rotation: { x: Math.PI / 4, y: 0, z: 0 } },
          React.createElement("coneGeometry", { args: [1.5, 3, 4] }),
          React.createElement("meshLambertMaterial", { color: 0xFFA500, flatShading: true })
        ),
        // Ear Tufts
        React.createElement("mesh", { position: [-5, 0, 17], rotation: { x: 0, y: 0, z: -0.5 } },
...
        ),
        // Legs
        React.createElement("mesh", { position: [-3, 0, -2], castShadow: true, receiveShadow: true },
            React.createElement("boxGeometry", { args: [2, 2, 4] }),
            React.createElement("meshLambertMaterial", { color: 0xFFA500, flatShading: true })
        ),
        React.createElement("mesh", { position: [3, 0, -2], castShadow: true, receiveShadow: true },
            React.createElement("boxGeometry", { args: [2, 2, 4] }),
...
  const car = useRef(null);
  // FIX: Pass the rowIndex to the animation hook to correctly calculate its world position.
  useVehicleAnimation(car, rowIndex, direction, speed);
...
  const truck = useRef(null);
  // FIX: Pass the rowIndex to the animation hook to correctly calculate its world position.
  useVehicleAnimation(truck, rowIndex, direction, speed);
...
// FIX: Accept rowIndex as an argument to correctly determine the vehicle's world Y position.
function useVehicleAnimation(ref, rowIndex, direction, speed) {
...
    const playerY = playerState.currentRow * tileSize;
    ...
    // FIX: Calculate the vehicle's actual Y position using the passed rowIndex.
    // The original code used vehicle.position.y, which was incorrect because
    // the vehicle's group is nested inside the row group, making its local y-position 0.
    const vehicleY = rowIndex * tileSize;
    const distanceBehindPlayer = playerY - vehicleY;

    // Only hide vehicles that are significantly behind the player
    if (distanceBehindPlayer > 8 * tileSize) {
      vehicle.visible = false;
      return; // Stop further processing for this vehicle
    } else {
...
        event.preventDefault();
        if (gameStatus === 'running') queueMove("forward");
      } else if (event.key === "ArrowDown") {
        event.preventDefault();
        if (gameStatus === 'running') queueMove("backward");
      } else if (event.key === "ArrowLeft") {
        event.preventDefault();
        if (gameStatus === 'running') queueMove("left");
      } else if (event.key === "ArrowRight") {
        event.preventDefault();
        if (gameStatus === 'running') queueMove("right");
